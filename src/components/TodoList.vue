    ...
    <div>
      <el-table :data="todoTableData">
        ...
        <el-table-column prop="price">
          <template slot-scope="scope">
            <el-input v-if="scope.row.isEdit" v-model="scope.row.price"></el-input>
            <span v-else>{{scope.row.price}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="operation">
          <template slot-scope="scope">
            <el-button v-if="scope.row.isEdit" type="primary" icon="el-icon-check" circle  @click="editRow(scope.row)"></el-button>
            <el-button v-else type="warning" icon="el-icon-edit" circle @click="editRow(scope.row)"></el-button>
...
export default {
  ...
  computed: {
    ...
    ...mapState({
      testObj: state => state.todoList.testObj,
      todoForm: state => state.todoList.todoForm,
      todoTableData: state => state.todoList.todoTableData
    })
  },
  mounted() {},
  methods: {
